  ...
  return {
    ...
    execute: function () {
      ...
      _export("GamePlay", GamePlay = (_dec = ccclass('GamePlay'), _dec2 = property(_crd && UIController === void 0 ? (_reportPossibleCrUseOfUIController({
        ...
        activeAimTargets() {
          for (var index = 0; index < this.UIController.AimTargets.length; index++) {
            var node = this.UIController.AimTargets[index];
        ...
        checkHitEnemy() {
          var pointBdx = this.UIController.point.getComponent(UITransform).getBoundingBoxToWorld();

          for (var index = 0; index < this.UIController.Enemies.length; index++) {
            var node = this.UIController.Enemies[index];
            var enemyBdx = node.getComponent(UITransform).getBoundingBoxToWorld();

            if (pointBdx.intersects(enemyBdx)) {
              node.active = false;
              this.isHit[index] = true;
              var enemyLife = this.UIController.EnemyCounts[index];
          ...
          var isHit = this.isHit.find(item => item === true);
