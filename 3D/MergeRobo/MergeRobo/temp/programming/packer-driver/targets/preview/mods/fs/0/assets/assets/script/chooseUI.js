    ...
    execute: function () {
      ...
      _export("ChooseUI", ChooseUI = (_dec = ccclass('ChooseUI'), _dec2 = property(Node), _dec3 = property(_crd && reuseList === void 0 ? (_reportPossibleCrUseOfreuseList({
        ...
        start() {
          ...
          var key = Object.keys(this.path).sort();
        ...
        _getAllPrefabPaths() {
          ...
          var paths = {};
          var added = [];
          var pathToUuid = (_assetManager$resourc = assetManager.resources) === null || _assetManager$resourc === void 0 ? void 0 : _assetManager$resourc.config.paths._map;
          var resPaths = Object.keys(pathToUuid);
          var {
            length
          } = resPaths;

          for (var i = 0; i < length; i += 1) {
            var aliasPath = resPaths[i];
            var aliasArr = aliasPath.split('/');
            var name = aliasArr[0];
            if (name !== this.resouceType) continue;
            var entryObj = pathToUuid[aliasPath];
            var entry = entryObj;
            var getTarget = false;
        ...
        showFBX(txt) {
          ...
          (_assetManager$getBund = assetManager.getBundle('resources')) === null || _assetManager$getBund === void 0 ? void 0 : _assetManager$getBund.load(this.path[txt], Prefab, (err, prefab) => {
            ...
            var arrParticle = node.getComponentsInChildren(ParticleSystemComponent);
