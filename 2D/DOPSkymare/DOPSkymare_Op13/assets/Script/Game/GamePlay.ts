...
export default class GamePlay extends cc.Component {
    ...
    @property(cc.Node)
    OS_Pomni: cc.Node = null;
    @property(cc.Node)
    OS_Chibi: cc.Node = null;
    @property(cc.Node)
    OS_BG: cc.Node = null;
    ...
    private registerEvent(): void {
        //    this.OS_Chibi.on(cc.Node.EventType.TOUCH_START, this.chibiTouchStart, this);
        this.btn_install.on(cc.Node.EventType.TOUCH_START, this.GameController.installHandle, this);
    }


    private chibiTouchStart(): void {
        this.openScene.active = false;
    ...
    private hidePomniHint(): void {
        if (Constants.currentLv === 1) {
            this.Pomni_Lv1.active = false;
            this.Pomni_Lv1.parent.getChildByName("overlay").active = false;
        } else if (Constants.currentLv === 2) {
            this.Pomni_Lv2.active = false;
            this.Pomni_Lv2.parent.getChildByName("overlay").active = false;
        } else {
            this.Pomni_Lv3.active = false;
            this.Pomni_Lv3.parent.getChildByName("overlay").active = false;
        }
    }
    ...
    private handleDoneLevel(): void {
        if (Constants.currentLv === 1) {
            ...
            // this.default_Level1.active = false;
    ...
    protected update(dt: number): void {
        if (Constants.Responsive.currentDevice === "vertical_mobile") {
            if (!Constants.isDoneLevel1) {
                ...
                Constants.Responsive.calculated = 10;
            }
        } else if (Constants.Responsive.currentDevice === "horizon_Tablet") {
            Constants.Responsive.calculated = 30;
        }
