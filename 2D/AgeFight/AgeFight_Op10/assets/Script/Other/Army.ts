...
export default class Army extends cc.Component {
    ...
    private goToTankPoint(): void {
        let targetPoint = Constants.tank_TroopPoints[Constants.troopsSpawnLeftCount];
        
        let worldPos = targetPoint.convertToWorldSpaceAR(cc.Vec3.ZERO);

        let targetPos = this.node.parent.convertToNodeSpaceAR(worldPos);
        Constants.troopsSpawnLeftCount -= 1;
    ...
    private checkDoneAttackAnim(isTank: boolean, o:cc.PhysicsCollider): void {
        this.spine.setCompleteListener((trackEntry: sp.spine.TrackEntry) => {
            if (trackEntry.animation.name === "Attack") {

                if(o.tag !== 1) {
                    let VFx_bleed = o.node.getChildByName("Spine").getChildByName("Vfx_1").getComponent(cc.Animation);
                    VFx_bleed.play();
                }

                if(isTank) {
                    let TankComp = o.getComponent(TankController)
    ...
    private handleDecreaseHealthArmy(): void {
        if (this.skin === "ally") {
            ...
            if (this.node.name === "Army2_Lv2") {
                Constants.allyArmies[1].getComponent(Army).HealthBar.scaleX -= 0.5;
                this.AudioManager.playSound(Constants.SoundTrack.Lv2_Gun);
                return;
            }

            if (this.node.name === "Army2_Lv1") {
                Constants.allyArmies[1].getComponent(Army).HealthBar.scaleX -= 0.3;
                this.AudioManager.playSound(Constants.SoundTrack.Lv2_Kinife);
                return;
            }
    ...
    private die(): void {
        let VFx_bleed = this.node.getChildByName("Spine").getChildByName("Vfx_1").getComponent(cc.Animation);
    ...
    private checkRun(): void {
        ...
        if (this.skin === "enemy" && this.canRun) {
            ...
            if(Constants.isRotate) {
                targetPos = new cc.Vec2(currentPos.x - 30 - Constants.Responsive.speedCalculated, firstEnemyPos.y);
            } else {
                targetPos = new cc.Vec2(currentPos.x - 20 - Constants.Responsive.speedCalculated, firstEnemyPos.y);
            }
